...
- [x] Towers and enemies are prefabs (`Assets/Prefabs`) driven by `TowerData` / `EnemyData` / `WaveSet` assets in `Assets/Data`. `GameDataBuilder` (no menu item now; called by the tower importer) creates missing assets/prefabs and wires the managers. Also fixed double tower registration and the sell-refund mismatch.
...
## Kept on purpose
The importers in `Assets/Scripts/Editor` (`WaveImporter`, `TowerImporter`, `RouteImporter`, `TerrainImporter`) are hand-run tools that turn the extracted WC3 map files into scene data; only the tower, wave and terrain ones keep a Tools menu entry. `RouteImporter` replaced the old guessed spawner generator and its patch-up tools. The region dump tool and `region_dump.txt` were removed.
...
- [ ] Multiplayer (LAN, lockstep). Done: fixed tick, per-player state, commands, state checksum, TCP lockstep session, lobby menu, drop hand-over (checked by `NetCheck` over loopback). Not yet tried between two real game instances. To do: per-player UI (owner colours on towers, trade-gold UI, player list), builder-unit start spots near each player's spawn, interpolation of creep/projectile movement between ticks, HUD countdown from `GameManager.WaveCountdown`, reconnecting, mid-game pause.
