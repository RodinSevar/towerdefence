...
- [x] Spawners and routes come from the map's triggers (15 spawners, 6 groups, region-triggered orders), creeps follow `RouteStep`s and turn at region entry; `yellow_right` = 1 creep; creeps need a granted path for every order (`EnemyManager.pathRequestsPerSecond`, 150/s FIFO, calibrated to the real game's ~2 s start delay and pauses; real scheduling is engine-decompile question 9). Open: the map gives gray's creeps no orders yet they move in the real game (we route them straight to the exit; decompile question 10); `Prevent Attack` (a creep that attacks is redirected to the exit after 1 s) is not implemented.
