...
- **Spawners and routes** are now generated from the map's triggers (`Tools > Import WC3 Routes`): 15 spawn regions, 6 unit groups, each group's chain of region-triggered move orders ending at the Load region; `yellow_right` spawns a fixed 1 creep. Two behaviours are modelled from what is seen in the real game, not from the map's script: (1) the gray player's creeps are created at `Left Move 2` and the script never gives them a group or an order, but in the real game they do leave their spawn and walk down, so we route them straight to the exit (the mechanism is unknown; player 12 is a computer player with no AI script, so an engine default is suspected); (2) creeps do not all start at once: in the real game they wait a couple of seconds after spawning and pause again at intervals, consistent with move orders going through a rate-limited pathfinding request queue. `EnemyManager.pathRequestsPerSecond` (150) models this: every new order needs a granted path, served FIFO at that rate. The rate is calibrated by eye (~280 creeps start over ~2 s); the real scheduling is a question for the engine decompile (see the handoff). Some levels also change lives (-10, -19, +20, +80 in the triggers); not implemented.
